...
def create_interactive_blendshape_plot(blendshapes_data, blendshape_name='eyeBlinkRight'):
    """
    Create an interactive blendshape plot with a draggable slider.
    This version uses a different approach that's more responsive.
    """
    ...
    # Add a vertical line that will move with the slider
    fig.add_trace(
        go.Scatter(
            x=[0, 0],
    ...
    # Update layout
    fig.update_layout(
        title=f'{blendshape_name} Blendshape Values Over Time',
        height=600,
        width=1000,
        showlegend=True,
        xaxis_title='Frame Number',
        yaxis_title=f'{blendshape_name} Value',
        hovermode='x unified'
    )
            ...
            'steps': [
                {
                    ...
                    'label': str(i),
                    'method': 'animate'
                } for i in range(0, len(blendshapes_data), max(1, len(blendshapes_data)//50))
            ]
        }]
    )
    ...
    frames = []
    for frame_idx in range(0, len(blendshapes_data), max(1, len(blendshapes_data)//100)):
        frame_data = [
            # Blendshape line
            go.Scatter(
                x=frame_numbers,
...
def create_video_slider_plot(video_path, face_crop=True, crop_margin=50):
    """
    Create a simple Plotly plot with only video frames and a slider.
    ...
    # Calculate face crop dimensions (assuming face is roughly in center)
    if face_crop:
        # Estimate face region (center 1/3 of frame)
        ...
        print(f"Data reduction: {original_width * original_height / (width * height):.1f}x")
    else:
    ...
    # Update layout
    title = 'Video Player with Frame Slider'
    if face_crop:
        title += f' (Face Cropped)'
    ...
    fig.update_layout(
        sliders=[{
            ...
            'x': 0.1,
            'xanchor': 'left',
            'y': -0.25,  # Position slider below the figure
            'yanchor': 'bottom',
            'steps': [
                {
                    'args': [[i], {
                    ...
                    'label': str(i),
                    'method': 'animate'
                } for i in range(0, total_frames, max(1, total_frames//50))  # Show max 50 labels
            ]
        }]
    )
    ...
    # Create frames for animation
    frames = []
    for frame_idx in range(0, total_frames, max(1, total_frames//100)):  # Max 100 frames for performance
        # Read video frame
        cap = cv2.VideoCapture(str(video_path))
...
# idx_lst = random.sample(range(0, len(df)+1), 10) #[1004, 2877,1744, 3663, 556] #random.sample(range(0, len(df)+1), 50)
# idx_lst = [1004, 2877,1744, 3663, 556] #random.sample(range(0, len(df)+1), 50)
idx_lst = [623, 4615, 1420] #random.sample(range(0, len(df)+1), 50)
idx_lst = [3663] #random.sample(range(0, len(df)+1), 50)
for i in tqdm(idx_lst):
    row = df.iloc[i]
    data_path = parent_path / row['run_path']
